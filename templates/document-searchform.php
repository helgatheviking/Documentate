...
<form role="search" method="get" class="searchform documentate-document-search" action="<?php echo home_url( '/' ); ?>" autocomplete="off">
	...
	<input type="text" placeholder="<?php _e( 'Search Documents...', 'documentate' ); ?>" value="<?php echo get_search_query(); ?>" name="s" id="s" title="<?php echo esc_attr_x( 'Search for:', 'label', 'woocommerce' ); ?>" />
</form>